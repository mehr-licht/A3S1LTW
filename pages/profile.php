...
<script type="text/javascript" src="../js/eye.js"></script>
 ...
 "../res/default.gif" ?> width="8%" class="avatar">
  ...
  <form id="profile-form" class="profile form" action="../actions/action_update.php" method="post">
    ...
    <?php if ($editable) { ?>
    <label id="profile-email" class="profile editable">email: </label><div contenteditable="false"><input class="profile editable" id="input-email"
          type="text" name="email" value="<?= $user_array[0]['email'] ?>" oninput="clearInputError('input-email')"required><span></span> </div>
    </p>

    <label id="profile-name" class="profile editable">name: </label><div contenteditable="false">
      <input class="profile editable" id="input-name" type="text" name="name" value="<?= $user_array[0]['name'] ?>" oninput="clearInputError('input-name')"><span></span> </div>
    </p>


    <label id="profile-birthday" class="profile editable">birthday: </label><div contenteditable="false">
       <input class="profile editable" id="input-birthday" type="text" name="birthday" value="<?= $user_array[0]['birthday'] ?>" oninput="clearInputError('input-birthday')"><span></span> </div>
    </p>
    ...
    if (isset($user_array[0]['birthday'])) {
      ...
      ?>
    <div contenteditable="false">
      <label id="profile-age" class="profile">age: <?php if ($age!== NULL) { echo $age; } ?></label></p>
    </div>
    <?php   
}
...
if ($editable) { ?>
    ...
    <label id="profile-address" class="profile editable">street: </label><div contenteditable="false">
       <input class="profile editable" id="input-address" type="text" name="street" value="<?= $user_array[0]['street'] ?>" oninput="clearInputError('input-address')"><span></span> </div>
    </p>

    <label id="profile-zip" class="profile editable">zip code: </label><div contenteditable="false">
        <input class="profile editable" id="input-zip" type="text" name="zipcode" value="<?= $user_array[0]['zipcode'] ?>" oninput="clearInputError('input-zip')"><span></span> </div>
    </p>

    <?php 
  } ?>

    <label id="profile-city" class="profile editable">city: <?php if (!$editable) $user_array[0]['city']  ?> </label>
    <div contenteditable="false">
<?php if ($editable){?><input class="profile editable" id="input-city" type="text" name="city" value="<?= $user_array[0]['city'] ?>" oninput="clearInputError('input-city')"><?php } ?><span></span> </div>
    </p>

    <label id="profile-country" class="profile editable">country: <?php if (!$editable) $user_array[0]['country']  ?></label>
    <div contenteditable="false">
    <?php if ($editable){?><input class="profile editable" id="input-country" type="text" name="country" value="<?= $user_array[0]['country'] ?>" oninput="clearInputError('input-country')"><?php } ?><span></span> </div>
    </p>

    <?php if ($editable) { ?>
    <label id="profile-phone" class="profile editable">phone: </label><div contenteditable="false">
        <input class="profile editable" id="input-phone" type="text" name="phone" value="<?= $user_array[0]['phone'] ?>" oninput="clearInputError('input-phone')"><span></span> </div>
    </p>
    <div class="profile prof" id="profileEdit"> 
      <button onclick="return validateInputs()" type="submit" class="button" formaction="../actions/action_update.php" formmethod="post">Save</button>
    </div>
    <?php 
  } ?>
  </form>
